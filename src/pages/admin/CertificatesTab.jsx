...
const CertificatesTab = () => {
  ...
  const [form, setForm] = useState({ name: '', issuer: '', date: '', description: '', link: '' });
  ...
  const openAdd = () => {
    setEditingItem(null);
    setForm({ name: '', issuer: '', date: '', description: '', link: '' });
    setModalOpen(true);
  };

  const openEdit = (item) => {
    setEditingItem(item);
    setForm({ name: item.name, issuer: item.issuer, date: item.date, description: item.description, link: item.link || '' });
    setModalOpen(true);
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/admin/certificates/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
        ...
        <div className="grid gap-4">
          {items.map((item) => (
            <div key={item._id} className="bg-zinc-800 rounded-xl p-5 ring-1 ring-zinc-50/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-medium text-zinc-50">{item.name}</h3>
                <p className="text-zinc-400 text-sm mt-1">{item.issuer} • {item.date}</p>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
          <div className="bg-zinc-800 rounded-2xl p-6 w-full max-w-lg ring-1 ring-zinc-50/5 max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-semibold text-zinc-50 mb-4">{editingItem ? 'Edit' : 'Add'} Certificate</h3>
